...
import { parseCommand, splitTopLevelArgs, type GgbObject } from '@/lib/math/tikz-export/ggb-to-tikz';
...
export const GEOGEBRA_PLUGIN_SET_DIGEST =
  'geogebra-command-projector/v1' as const;
...
export function projectGeogebraCommandsToGeometryDoc(input: {
  ...
  const consumedStatements = new Set(definitions.map((item) => item.statement.index));
  for (const statement of statements) {
  ...
  const bindings = [...definitionBindings, ...styleBindings]
  ...
  const constraints = definitions.flatMap((item) => item.constraint ? [item.constraint] : []);
