...
const scene: Scene = {
  ...
  elements: [
    ...
    { stableId: 'circle-1', kind: 'circle', center: { x: 2, y: 0 }, radius: 1.5, radiusSource: null, definition: null, stmtIndex: 3, refs: [], style: DEFAULT_STYLE },
...
describe('hit-test', () => {
  ...
  it('按照标签实际偏移后的可见边界命中', () => {
    const naturalViewport = { ...viewport, scale: NATURAL_CM_TO_CSS_PX };
    expect(hitTestElement({ x: 0, y: -10 }, scene, naturalViewport, 2)).toMatchObject({
      stmtIndex: 4,
      kind: 'label',
    });
    expect(hitTestElement({ x: 0, y: 5 }, scene, naturalViewport, 2)?.kind).not.toBe('label');
  });
});
