...
/* Deprecated implementation retained temporarily for patch-safe migration.
...
export function parseAgentMessageWidget(value: unknown): AgentMessageWidget | null {
...
export { AgentMessageWidgets, parseAgentMessageWidget } from './agent-message-widgets-v2';
export type { AgentMessageWidget } from './agent-message-widgets-v2';

/*
const PLOT_COLORS: Record<FunctionPlotWidget['series'][number]['color'], string> = {
  blue: '#007aff', red: '#ff3b30', green: '#34c759', orange: '#ff9500',
  purple: '#af52de', gray: '#8e8e93',
};
...
function FunctionPlotCard({ widget }: { widget: FunctionPlotWidget }) {
  ...
  return (
    <section className="tz-agent-widget tz-agent-widget--plot">
      <header><strong>{widget.title}</strong><code>{widget.expression}</code></header>
      <svg viewBox="0 0 320 180" role="img" aria-label={`${widget.title} 函数图`}>
        ...
        {minY <= 0 && maxY >= 0 ? <line x1="18" y1={mapY(0)} x2="302" y2={mapY(0)} className="tz-function-plot__axis" /> : null}
        {minX <= 0 && maxX >= 0 ? <line x1={mapX(0)} y1="18" x2={mapX(0)} y2="162" className="tz-function-plot__axis" /> : null}
      ...
      <div className="tz-function-plot__controls">
        <label>缩放 <input type="range" min="1" max="4" step="0.25" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></label>
...
function GeometryFlowCard({ widget }: { widget: GeometryFlowWidget }) {
  ...
  return (
    <section className="tz-agent-widget tz-agent-widget--flow">
      <header><strong>{widget.title}</strong>{widget.source ? <small>{widget.source}</small> : null}</header>
      ...
      {active ? (
        <div className="tz-geometry-flow__detail" role="tabpanel">
          <strong>{active.title}</strong><p>{active.explanation}</p>
  ...
  return (
    <div className="tz-agent-widgets" aria-label="AI 结果">
      {widgets.map((widget, index) => {
        if (widget.kind === 'code-example') {
          return (
            <details className="tz-agent-widget" key={`${widget.kind}:${index}`}>
              <summary><strong>{widget.title}</strong><span>{widget.lineCount} 行 · 默认折叠</span></summary>
        ...
        if (widget.kind === 'mutation') {
          return (
            <section className="tz-agent-widget tz-agent-widget--success" key={`${widget.kind}:${index}`}>
              <strong>{widget.title}</strong><p>{widget.detail}</p>
